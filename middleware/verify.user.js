...
isPasswordAndUserMatch = (req, res, next) => {
  AuthModel.findByEmail(req.body.email)
      .then((user)=>{
          if(user.length){
              user = user[0];
              authHelper.comparePassword(req.body.secret, user['password'])
                ...
                .catch(function(error){
                    return helper.sendError(res, 400, "Email & passwords combination doesn't match!");
                });
          }
      })
      .catch(function(err){
...
isEmailExists = (req, res, next) => {
  AuthModel.findByEmail(req.body.email)
      .then((user)=>{
          if( user[0].length ){
            return helper.sendError(res, 409, "User already exists with given Email! Try forgot password!");
          }
          return next();
      })
      .catch(function(err){
...
isEmailExistsInDB = (req,res, next) => {
  AuthModel.findByEmail(req.params.email)
    .then((user)=>{
        ...
        return helper.sendError(res, 409, "User doesn't Exists, Please Check your email again!");
    })
    .catch(function(err){
        console.log("err",err.sqlMessage);
        return helper.sendError(res, 400, "Some Internal Error Occurred!");
...
exports.validateUpdatePassData = (req, res, next) => {
  ...
  if( req.body.email !== req.params.email )
    ...
    return helper.sendError(res, 400, "Invalid email provided");
      ...
      return helper.sendError(res, 400, "Something Wrong happened Please try again later");
